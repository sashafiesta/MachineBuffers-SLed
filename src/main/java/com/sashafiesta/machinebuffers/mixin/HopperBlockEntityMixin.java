...
public abstract class HopperBlockEntityMixin {
	...
	private static int getCooldown(int initialValue, HopperBlockEntity blockEntity) {
		if (blockEntity instanceof MachineBufferBlockEntity)
			return 1;
		return initialValue;
	}
	...
	private int modifyInventorySize(int initialValue) {
		if ((Object) this instanceof MachineBufferBlockEntity)
			return 1;
		return initialValue;
	}
	...
	private static void setLongerTransferCooldown(HopperBlockEntity blockEntity, int cooldown) {
		((HopperBlockEntityMixin) (Object) blockEntity).setTransferCooldown(cooldown + (blockEntity instanceof MachineBufferBlockEntity ? 4 : 0));
	}
